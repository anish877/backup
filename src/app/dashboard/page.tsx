...
const Dashboard = () => {
  ...
  const latestLog: HealthLog | null = dailyLogs.length > 0 ? dailyLogs[dailyLogs.length - 1] : null;
  const latestFeedback: FeedbackItem | null = feedback.length > 0 ? feedback[feedback.length - 1] : null;
              ...
              <CardContent className="bg-white">
                ...
                <div className="mt-4 bg-orange-50 p-3 rounded-lg">
                  <p className="text-sm text-gray-700">
                    ...
                    {dailyLogs.length >= 3 ? 
                      `Your nutrition scores are ${dailyLogs.slice(-3).every(log => (log.nutrition || 0) > 70) ? 'consistently good' : 'showing room for improvement'}. Focus on ${getRecommendations()[0] ? getRecommendations()[0].toLowerCase() : 'maintaining your good habits'}.` : 
